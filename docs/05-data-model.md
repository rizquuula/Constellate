...
| 0004 | `0004_operator_sessions.sql` | New `operator_sessions(id PK, created_at, expires_at, last_seen_at)` + `idx_operator_sessions_expires` | Server-side sessions behind the opaque `constellate_session` cookie (`session_ttl`, default 24 h). `expires_at` **slides** — each validated request pushes it to `now + session_ttl`; `last_seen_at` records the touch. |
